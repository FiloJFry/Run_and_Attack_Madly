class Nemico
{
    ...
    {   
        if(!DatiDiPosizione.InPausa)
        {
            ...
            Boss.setAttribute('src',`./Nemici/${this.nome}_attaccando.jpg`);
            FrasiNemico.textContent = `${this.Frasi[0]}`;
            AttaccoNemico.style.color = this.coloreAttacco;
            setTimeout(() => {Boss.setAttribute('src',`./Nemici/${this.nome}.jpg`); FrasiNemico.textContent = "";},1000);
...
{   
    ...
    {    
        if(this.munizioni > 0)
        { 
            ArmaInCanna.setAttribute('src',`./Armi/${this.nome}_attaccando.jpg`);
            this.munizioni -= 1;
            PiuInfo.textContent = `${this.munizioni}|${this.inventario}`;
            RumoriArma.textContent = `${this.Rumori[0]}`; 
            setTimeout(() => {ArmaInCanna.setAttribute('src',`./Armi/${this.nome}.jpg`); RumoriArma.textContent = "";},100);
        }
        else
        {   
    ...
    Step(a,p)
    {
        ...
        setTimeout(() => {ArmaInCanna.setAttribute('src',`./${a}/${this.nome}${p}.jpg`); ArmaInCanna.classList.remove('slideOutDown'); ArmaInCanna.classList.add('slideInUp'); if(p != ""){RumoriArma.textContent = `${this.Rumori[p.split('a')[2]]}`;} else{RumoriArma.textContent = "";}},500);
...
{   
    Spara()
    {
        if(this.munizioni == 1)
        {
            ArmaInCanna.setAttribute('src',`./Armi/${this.nome}_attaccando.jpg`);
            this.munizioni = 0;
            setTimeout(() => {ArmaInCanna.setAttribute('src',`./Armi/${this.nome}.jpg`);},100);
            this.Ricarica();
        }
        else
        {
